...
import static io.prestosql.plugin.jdbc.DecimalPropertiesProvider.getDecimalRounding;
import static io.prestosql.plugin.jdbc.DecimalPropertiesProvider.getDecimalRoundingMode;
    ...
    public Optional<ColumnMapping> toPrestoType(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        if (typeHandle.getJdbcType() == Types.DECIMAL && getDecimalRounding(session) == ALLOW_OVERFLOW) {
            ...
            if (precision > Decimals.MAX_PRECISION) {
                int scale = calcDecimalScaleMapping(precision, typeHandle.getDecimalDigits(), getDecimalDefaultScale(session));
