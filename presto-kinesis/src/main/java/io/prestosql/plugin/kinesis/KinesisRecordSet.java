            ...
            implements RecordCursor
    {
        ...
        public boolean advanceNextPosition()
        {
            ...
            if (listIterator.hasNext()) {
                ...
                log.debug("(%s:%s) Read all of the records from the shard:  %d batches and %d messages and %d total bytes.", split.getStreamName(), split.getShardId(), batchesRead, totalMessages, totalBytes);
                return false;
            }
        }
                ...
                throws ResourceNotFoundException
        {
            ...
            while (!fetchedRecords && attempts < fetchAttempts) {
                ...
                if (isLogBatches) {
                    log.info("(%s:%s) Fetched %d records from Kinesis.  MillisBehindLatest=%d Attempt=%d", split.getStreamName(), split.getShardId(), kinesisRecords.size(), getRecordsResult.getMillisBehindLatest(), attempts);
                }
        ...
        private boolean nextRow()
        {
            ...
            log.debug("(%s:%s) Reading record with partition key %s", split.getStreamName(), split.getShardId(), partitionKey);
